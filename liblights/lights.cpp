...
int set_backlight_light(struct light_device_t* dev, struct light_state_t const* state)
{
	...
	//* add by bonovo zbiao for android box
	//***********************************************
    int dwByteWrite, i, sum;
	unsigned char brightness_value[8] = {0xFA, 0xFA, 0x08, 0x00, 0x82, 0x00, 0x00,0x00};
	//***********************************************
	...
	//* add by bonovo zbiao for android box
	//***********************************************
	brightness_value[5] = brightness;
	for(i = 0, sum = 0; i < 6; i++)
	{
		sum += brightness_value[i];
	}
	brightness_value[6] = sum&0x00ff;
	brightness_value[7] = (sum>>8)&0x00ff;
    dwByteWrite = write(ttyS3_fd, brightness_value, sizeof(brightness_value));
    //LOGD("==============bonovo dev/ttyS3 open result========ttyS3_fd:%d brightness:%d\n",ttyS3_fd,brightness);
	//***********************************************
...
static int light_device_open(const struct hw_module_t* module, const char* name, struct hw_device_t** device)
{
    ...
    *device = &dev->common;
    ...
    if((ttyS3_fd = open("/dev/ttyS3", O_RDWR|O_NOCTTY|O_NONBLOCK)) < 0) {
        LOGI("==============bonovo open ttyS3 Mesg:%s\n",strerror(errno));
        LOGI("==============bonovo dev/ttyS3 open result = %d\n",ttyS3_fd);
    }
	//***********************************************
